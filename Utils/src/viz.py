...
def draw_trajectories(canvas: Player, tr: Recording):
...
def redraw_positions(player: Player, recording: Recording, frame, ovals=None, orint=None):
    if ovals is None:
        ovals = dict()
    if orint is None:
        orint = dict()
    
    for agent_id, infos in recording.pos.items():
        agent_id = int(agent_id)

        if frame not in infos:
            if agent_id in ovals:
                player.hide_item(ovals[agent_id])
            continue

        if agent_id in ovals:
            player.show_item(ovals[agent_id])

            player.move_circle(ovals[agent_id], infos[frame].pos)

            if not hide_orientation:
                player.move_orientation(orint[agent_id], infos[frame])
            
        else:
            ovals[agent_id] = player.circle(
                infos[frame].pos,
                infos[frame].R,
            ...
            if not hide_orientation:
                orint[agent_id] = player.orientation(infos[frame], "black")
...
def draw_mesh(player: Player, mesh: NavMesh, show_text=True):
    if show_text:
        for id, p in mesh.vertices.items():
            player.text(p, "{:0}, {:0}".format(p[0], p[1]), "#ccc")

    for id, node in mesh.nodes.items():
        for v0, v1 in pairwise(node.vertices):
            player.line(mesh.vertices[v0], mesh.vertices[v1], color="#ccc")
...
if __name__ == "__main__":
    ...
    recording = read_recording(args.source_file)

    player = Player(scale, recording.steps)

    if args.navmesh is not None:
        mesh = read_mesh(args.navmesh)
        draw_mesh(player, mesh, show_text=args.show_mesh_text)
    ...
    if not hide_trajectory:
        draw_trajectories(player, recording)

    result_redraw = redraw_positions(player, recording=recording, frame=0)
    ovals, orient = result_redraw
    player.set_redraw(lambda new_frame: redraw_positions(player, recording=recording, frame=int(new_frame), ovals=ovals, orint=orient))
